...
class Searchmodel extends CI_Controller {
	...
	public function cordinated_project($usid)
	{
		...
		$cor_pro = $this->Searchmodel->get_cord_proj($usid);
	...
	{
		$row = $this->Searchmodel->get_proj_details($pid);
	...
	public function checkform($usid)
	{
		...
		else
		{
			...
			$this->Searchmodel->modify($usid,$pass1,$name,$email);
	...
	public function ratesubmit($pid)
	{
		...
		else
		{
			...
			$this->Searchmodel->rateproject($pid,$rate1,$rate2,$rate3,$rate4,$rate5);
	...
	{	
		if($this->Searchmodel->passcheck($a,$abc)==true)
		{
			return true;
	...
	public function checkcreate($usid)
	{
		...
		else
		{
			...
			$email = $this->input->post('email');
			$this->Searchmodel->addfac($facid,$pass1,$name,$email);
			redirect(base_url().'index.php/SearchController/asdfg/'.$usid);
	...
	{
		$facid = $this->input->post('facid');
		if($this->Searchmodel->checkrepeat($facid)==true)
			return true;
		else
		{
